...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val mapRes = mutableMapOf<String, String>()
    mapRes.putAll(mapA)
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val res1 = chars.toSet()
    val res2 = word.toLowerCase().toSet()
    return (res1.map { it.toLowerCase() }.intersect(res2) == res2)
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (i in list) if (sum[i] == 1) sum.remove(i)
    return sum
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in 0 until list.size) {
        ...
        if (element in res) return Pair(res[element] ?: 0, i)
        else res[list[i]] = i
    }
    return Pair(-1, -1)
}
